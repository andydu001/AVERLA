...
public class Content extends ContentProvider {
    ...
    public Uri insert(@NonNull Uri NotInUse, @Nullable ContentValues contentValues) {
        ...
        if(sqLiteDatabase != null) {
         ...
         /*  curso = sqLiteDatabase.rawQuery("CREATE TABLE Resto (RES_ID int,   AFRICAN varchar (255), HAITIAN varchar (255), " +
                    "CUBAN varchar (255), JAMAICAN varchar (255), AMERICAN varchar (255), ASIAN varchar (255), EUROPEAN varchar (255), LATINO varchar (255))", null);
*/
            // curso = sqLiteDatabase.rawQuery("INSERT INTO Resto(RES_ID,AFRICAN,HAITIAN,CUBAN,JAMAICAN,AMERICAN,ASIAN,EUROPEAN,LATINO) VALUES ('1','Matsou Topo','Jacmel Capital Restaurant','El Rey De Polo','COOL SHADE','All American Food','Pekin Sea Food', 'Paris Cafe', 'Mexican Bar')", null);
            curso = sqLiteDatabase.rawQuery("SELECT * FROM Resto", null);
            Log.println(Log.WARN,"TAG", "My data base is open "+ sqLiteDatabase.isOpen());
